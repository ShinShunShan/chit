"""Q22: Password protection (CLI, PBKDF2-HMAC-SHA1, stdlib-only).

Standalone storage in q22_users.json so separate runs can login after register.
Usage:
  python q22.py register alice MyPassword1
  python q22.py login    alice MyPassword1
  python q22.py show
"""
import os, sys, json, time, hashlib, hmac

DB = os.path.join(os.path.dirname(__file__), 'q22_users.json')
...
if __name__=='__main__':
	if len(sys.argv) < 2:
		print('Usage:')
		print('  python q22.py register <user> <password>')
		print('  python q22.py login <user> <password>')
		print('  python q22.py show')
	else:
		cmd=sys.argv[1]
