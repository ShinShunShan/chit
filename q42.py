"""Q42: Password-protected website (standalone) to resist password cracking.
PBKDF2-HMAC-SHA256, password policy, simple rate limiting.
Run:
  python q42.py  # http://127.0.0.1:5005
"""
...
USERS={}; ATTEMPTS=defaultdict(list)
ITER=200_000; WINDOW=60; MAX_ATTEMPTS=5
...
HOME="""<h2>Home</h2><ul><li><a href=/register>Register</a></li><li><a href=/login>Login</a></li></ul>{% if 'user' in session %}<p>Logged in: {{ session['user'] }}</p>{% endif %}"""

def strong(p): return len(p)>=8 and any(c.isupper() for c in p) and any(c.islower() for c in p) and any(c.isdigit() for c in p)

def store(p): s=os.urandom(16); h=hashlib.pbkdf2_hmac('sha256', p.encode(), s, ITER); return ITER,s.hex(),h.hex()

def verify(rec,p): it,s_hex,h_hex=rec; s=bytes.fromhex(s_hex); h_exp=bytes.fromhex(h_hex); h=hashlib.pbkdf2_hmac('sha256', p.encode(), s, it); return hmac.compare_digest(h,h_exp)

def limited(ip): now=time.time(); ATTEMPTS[ip]=[t for t in ATTEMPTS[ip] if now-t<WINDOW]; return len(ATTEMPTS[ip])>=MAX_ATTEMPTS
